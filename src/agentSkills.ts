...
- KEYFRAME ORDER AND PROGRESSIVE WIDTH: KF1 is the closest hero opening, framed from mid-thigh to the top of the head so the face is clear and prominent. KF2 widens slightly to a knee-up product-proof frame. KF3 widens again to a near-full-body front three-quarter fit-check. KF4 becomes a complete head-to-toe side or back three-quarter proof. KF5 is the widest frame, showing the full model plus a modest amount of the locked real background for the final product display.
...
- CAMERA DISTANCE LOCK: KF1 is closest to camera and crops from mid-thigh to the top of the head, with the face large, sharp, and easy to recognize. From KF2 through KF5, widen the field of view progressively and monotonically: each keyframe must be visibly wider than the previous one. Use a smooth camera pull-back across every scene; never push in or make a later keyframe tighter. KF5 may reveal more of the real background but the model must remain visually dominant. Do not use an extreme distant wide shot or excessive empty background.
...
- COMPACT SCENE TEMPLATE: every scene must contain only the start/end keyframe sentence plus ACTION, CAMERA STYLE, and LIGHTING. ACTION must be one short physical model movement sentence only. Do not include camera movement, background, lighting, keyframe chain text, product identity, model identity, Master Prompt, Model Lock, Product Lock, Background Lock, Continuity Rule, LOCKS, CLEAN, negative prompt, or compliance notes inside ACTION or the scene body; the input images and keyframes already carry that detail.
